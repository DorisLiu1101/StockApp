/**
 * ==========================================
 * 維股利 後端 API (v22_名稱修改擴充版)
 * 更新日期: 2026-02-16
 * 版本說明: 
 * 1. 新增 'rename' 功能：支援前端修改股票名稱並寫入 B 欄
 * 2. 保持原有 'add', 'getReport' 功能運作正常
 * ==========================================
 */
  ...
  if (lock.tryLock(10000)) {
    try {
      ...
      if (action === 'rename') {
        ...
        if (!sheet) return ContentService.createTextOutput(JSON.stringify({success: false, message: "無此市場分頁: " + market})).setMimeType(ContentService.MimeType.JSON);
        ...
        if (rowIndex > 0) {
          ...
          return ContentService.createTextOutput(JSON.stringify({success: true, message: "名稱更新成功"})).setMimeType(ContentService.MimeType.JSON);
        } else {
          return ContentService.createTextOutput(JSON.stringify({success: false, message: "找不到該股票"})).setMimeType(ContentService.MimeType.JSON);
        }
      }

      // === 功能 C: 新增交易 (Add Transaction) ===
      if (action === 'add') {
        var market = payload.market;
        var symbol = String(payload.symbol).toUpperCase();
        var txName = payload.name || ""; // 取得可能傳入的名稱
        ...
        var oldQty = 0;
        var oldCost = 0;
        var stockName = txName; // 預設使用傳入名稱

        // 尋找現有持股
        for (var i = 1; i < data.length; i++) {
          if (String(data[i][0]).toUpperCase() === symbol) {
            rowIndex = i + 1;
            stockName = data[i][1]; // 若已存在，優先使用現有名稱
        ...
        if (rowIndex !== -1) {
          ...
          // 插入新行
          // 若有傳入 txName 則寫入，否則留空
          var newRow = [symbol, txName, "", "", "", "", "", newCost, newQty];
